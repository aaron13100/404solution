...
class ABJ_404_Solution_WordPress_Connector {
    ...
    static function process404() {
        ...
        // Note: is_admin() does not mean the user is an admin - it return true when the user is on an admin screen.
        // Note: The HTTP_CONNECTION "close" is generated by the browser, not a real user. If there's a request to 
        // close the connection then there was already a keep-alive that was handled, so ignoring the "close" request
        // avoids duplicates.
        if ((!is_404()) || (is_admin()) || strtolower($_SERVER['HTTP_CONNECTION']) == "close") {
            return;
        }
