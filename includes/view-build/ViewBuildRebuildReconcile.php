 ...
 *
 * Cross-collaborator calls flow through the collaboration context's stage,
 * recovery, and data bundles instead of a broad orchestrator dispatch surface.
 *
 */
class ABJ_404_Solution_ViewBuildRebuildReconcile extends ABJ_404_Solution_ViewBuildCollaborator {
    ...
    public function sweepStaleRebuildTransients(): void {
        ...
        $this->host->dataBoundary()->queryAndGetResults("DROP TABLE IF EXISTS `" . esc_sql($preaggTable) . "`",
            array('log_errors' => false));
        $this->host->dataBoundary()->queryAndGetResults("DROP TABLE IF EXISTS `" . esc_sql($tempTable) . "`",
            array('log_errors' => false));
    }
    ...
    public function rebuildViewDoneInBackground(): void {
        ...
        if ($this->host->recoveryServices()->foregroundLease()->foregroundViewBuildLeaseActive()) {
            $this->host->dataBoundary()->logger()->debugMessage(
                '[staged] rebuildViewDoneInBackground: deferring; '
                . 'foreground build lease active. Rescheduled.'
            );
            $this->host->recoveryServices()->cronScheduler()->scheduleViewDoneRebuild(ABJ_404_Solution_ViewBuildConfig::VIEW_BUILD_FOREGROUND_LEASE_SECONDS);
            return;
        }
        if (!$this->host->recoveryServices()->lockCoordinator()->acquireViewBuildLock()) {
            $this->host->dataBoundary()->logger()->debugMessage(
        ...
        try {
            ...
            if (!$isComplete) {
                ...
                $this->host->dataBoundary()->rebuildHealth()->recordSuccess();
            }
        } catch (Throwable $e) {
            ...
            $this->host->dataBoundary()->logger()->warn('[staged] background rebuild yielded an error: ' . $e->getMessage());
            if ($this->host->dataBoundary()->rebuildHealth() instanceof ABJ_404_Solution_RebuildHealthState) {
                $this->host->dataBoundary()->rebuildHealth()->recordFailure($e->getMessage(), $this->host->dataBoundary()->rebuildHealth()->classifyError($e->getMessage()));
            }
        } finally {
    ...
    public function reconcileStagedTablesAtRunnerStartup(): string {
        $tempDeletemeTable = $this->host->stageServices()->stagePipeline()->viewDeletemeTableName();
        $tempBuildTable    = $this->host->stageServices()->stagePipeline()->viewBuildTableName();
        $doneTable         = $this->host->stageServices()->stagePipeline()->viewDoneTableName();

        $action = 'none';
        $haveDeleteme = $this->host->stageServices()->stateProbe()->stagedTableExists($tempDeletemeTable);

        if ($haveDeleteme) {
            $r = $this->host->dataBoundary()->queryAndGetResults('DROP TABLE IF EXISTS `' . $tempDeletemeTable . '`',
                array('log_errors' => false)
            );
            $err = isset($r['last_error']) && is_string($r['last_error']) ? trim($r['last_error']) : '';
            if ($err === '' || !$this->host->stageServices()->stateProbe()->stagedTableExists($tempDeletemeTable)) {
                ...
                $this->host->recoveryServices()->hostFailureNotices()->setStagedBuildHaltNotice('orphan_deleteme', sprintf(
        ...
        $haveBuild = $this->host->stageServices()->stateProbe()->stagedTableExists($tempBuildTable);
        $haveDone  = $this->host->stageServices()->stateProbe()->stagedTableExists($doneTable);
        ...
        if ($haveBuild && !$haveDone) {
            if (!$this->bufferIntegrityPassesForPromote($tempBuildTable)) {
                ...
                $this->host->dataBoundary()->queryAndGetResults('DROP TABLE IF EXISTS `' . $tempBuildTable . '`',
                    array('log_errors' => false));
                return 'cleaned';
            }
            $sql = 'RENAME TABLE `' . $tempBuildTable . '` TO `' . $doneTable . '`';
            $r = $this->host->dataBoundary()->queryAndGetResults($sql, array('log_errors' => true));
            $err = isset($r['last_error']) && is_string($r['last_error']) ? trim($r['last_error']) : '';
            if ($err === '' && $this->host->stageServices()->stateProbe()->stagedTableExists($doneTable)) {
                $this->host->dataBoundary()->logger()->infoMessage(sprintf(
            ...
            $this->host->recoveryServices()->hostFailureNotices()->setStagedBuildHaltNotice('promote_build_failed', sprintf(
        ...
        if ($haveBuild && $haveDone) {
            $r = $this->host->dataBoundary()->queryAndGetResults('DROP TABLE IF EXISTS `' . $tempBuildTable . '`',
                array('log_errors' => false)
            );
            $err = isset($r['last_error']) && is_string($r['last_error']) ? trim($r['last_error']) : '';
            if ($err === '' || !$this->host->stageServices()->stateProbe()->stagedTableExists($tempBuildTable)) {
            ...
            $this->host->recoveryServices()->hostFailureNotices()->setStagedBuildHaltNotice('orphan_build', sprintf(
    ...
    public function reconcilePostStageElevenState(): bool {
        $viewDoneTable  = $this->host->stageServices()->stagePipeline()->viewDoneTableName();
        $viewBuildTable = $this->host->stageServices()->stagePipeline()->viewBuildTableName();

        if (!$this->host->stageServices()->stateProbe()->stagedTableExists($viewDoneTable)) {
            return false;
        }
        if ($this->host->stageServices()->stateProbe()->stagedTableExists($viewBuildTable)) {
            return false;
        }
        ...
        if (function_exists('update_option')) {
            update_option($this->host->stageServices()->viewDoneState()->viewDoneFreshnessOptionName(), $this->host->dataBoundary()->clock()->now(), false);
        }
        $this->host->stageServices()->progressOptions()->clearAllProgressOptions();
    ...
    public function bufferIntegrityPassesForPromote(string $bufferTable): bool {
        $bufferRows = $this->host->stageServices()->batchExecutor()->countViewBuildRows();
        if ($bufferRows <= 0) {
            return false;
        }
        $liveRows = $this->host->stageServices()->batchExecutor()->countLiveRedirects();
        if ($liveRows <= 0) {
    ...
    /**
     * Write-through cache helper: reconcile view_done with the live
     * wp_abj404_redirects table on the row set, without waiting for the
     * staged-rebuild pipeline (S1..S11) to complete.
     *
     * The staged pipeline is async by design: stages yield on time budget
     * and a single request may run only one or two stages before
     * scheduling the next cron tick. Between the source-table mutation
     * and S11 RENAME swap, view_done is stale: newly INSERTed redirects
     * are invisible, DELETEd rows linger, UPDATEd rows show old values.
     * Production users see this as "I added a redirect but it does not
     * appear in the list until I refresh in a few minutes"; the e2e
     * suite sees it as "created redirect must appear in table" flakes
     * under concurrent load.
     *
     * This method closes the visibility gap surgically on the affected
     * row set, using three operations that mirror the three source-table
     * mutation classes:
     *
