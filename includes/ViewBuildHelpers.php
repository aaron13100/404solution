...
class ABJ_404_Solution_ViewBuildHelpers extends ABJ_404_Solution_ViewBuildCollaborator {
    ...
    // SQL-mode + max_allowed_packet probe lives on the sibling
    // ABJ_404_Solution_ViewBuildSqlModeProbe collaborator. The
    // $sqlModeProbeCache property is declared there; cross-collaborator
    // access in sanitizeUrlBeforeInsert() below goes through the
    // orchestrator's reflection-routed __get. probeSqlModeForBuild(),
    // detectAndAdjustSqlMode(), attemptRelaxSqlModeForBuildConnection(),
    // sqlModeProbeOptionName(), and clearSqlModeProbeCache() are all on
    // that collaborator. PHP-runtime probe lives on ViewBuildPhpEnvProbe;
    // MySQL-session operational + DDL-safety probe lives on
    // ViewBuildSessionEnvProbe. clearAllProgressOptions() above calls
    // clearPhpEnvironmentProbeCache() and (via clearSqlModeProbeCache)
    // clearSessionVariablesProbeCache so a fresh build re-evaluates the
    // host on next entry.
