...
class ABJ_404_Solution_FrontendRequestPipeline {
    ...
    function process404() {
        ...
        if ($requestedURL != "") {
            ...
            if ($autoRedirectsAreOn) {
                ...
                if ($this->logic->userIsPluginAdmin() && isset($options['debug_mode']) && $options['debug_mode'] == '1') {
                    $traceResult = $this->runMatchingEnginesWithTrace($matchRequest);
                    $this->renderDebugInterstitial($requestedURL, $traceResult, $options);
                    return;
                }
    ...
    /**
     * Like runMatchingEngines() but collects ALL engine results into a trace array.
     *
     * @param ABJ_404_Solution_MatchRequest $request
     * @return array{winner: ?ABJ_404_Solution_MatchResult, trace: array<int, array{engine: string, skipped: bool, result: ?ABJ_404_Solution_MatchResult, excluded: bool, error: string}>}
     */
    public function runMatchingEnginesWithTrace(ABJ_404_Solution_MatchRequest $request): array {
        $trace = [];
        $winner = null;
    ...
    /**
     * Render a standalone debug interstitial page (not inside WP admin chrome).
     *
     * @param string $requestedURL
     * @param array{winner: ?ABJ_404_Solution_MatchResult, trace: array<int, array{engine: string, skipped: bool, result: ?ABJ_404_Solution_MatchResult, excluded: bool, error: string}>} $traceResult
     * @param array<string, mixed> $options
     * @return void
     */
    private function renderDebugInterstitial(string $requestedURL, array $traceResult, array $options): void {
        if (function_exists('status_header')) {
            status_header(200);
        }

        $html = ABJ_404_Solution_Functions::readFileContents(__DIR__ . '/html/debugInterstitial.html');

        $html = $this->f->str_replace('{REQUESTED_URL}', esc_html($requestedURL), $html);

        // Build engine rows
        $rows = '';
        foreach ($traceResult['trace'] as $entry) {
            $engineName = esc_html($entry['engine']);
            $result = $entry['result'];

            if ($entry['error'] !== '') {
                $statusHtml = '<span class="status-error">Error</span>';
                $score = '-';
                $title = esc_html($entry['error']);
                $link = '-';
            } elseif ($entry['skipped']) {
                $statusHtml = '<span class="status-skipped">Skipped</span>';
                $score = '-';
                $title = '-';
                $link = '-';
            } elseif ($result === null || $result->getLink() === '') {
                $statusHtml = '<span class="status-no-match">No match</span>';
                $score = '-';
                $title = '-';
                $link = '-';
            } elseif ($entry['excluded']) {
                $statusHtml = '<span class="status-excluded">Excluded</span>';
                $score = number_format($result->getScore(), 1);
                $title = esc_html($result->getTitle());
                $link = esc_html($result->getLink());
            } else {
                $statusHtml = '<span class="status-matched">Matched</span>';
                $score = number_format($result->getScore(), 1);
                $title = esc_html($result->getTitle());
                $link = '<a href="' . esc_url($result->getLink()) . '">' . esc_html($result->getLink()) . '</a>';
            }

            $rows .= '<tr><td>' . $engineName . '</td><td>' . $statusHtml . '</td><td>' .
                $score . '</td><td>' . $title . '</td><td>' . $link . '</td></tr>';
        }
        $html = $this->f->str_replace('{ENGINE_ROWS}', $rows, $html);

        // Winner section
        $winner = $traceResult['winner'];
        if ($winner !== null) {
            $redirectCode = isset($options['default_redirect']) && is_scalar($options['default_redirect']) ? (string)$options['default_redirect'] : '301';
            $winnerDetails = esc_html($winner->getEngineName()) . ' &rarr; <a href="' .
                esc_url($winner->getLink()) . '">' . esc_html($winner->getTitle()) . '</a>' .
                ' (score: ' . number_format($winner->getScore(), 1) . ', ' . $redirectCode . ' redirect)';
            $html = $this->f->str_replace('{WINNER_CLASS}', '', $html);
            $html = $this->f->str_replace('{WINNER_LABEL}', 'Winner', $html);
            $html = $this->f->str_replace('{WINNER_DETAILS}', $winnerDetails, $html);

            $actionLinks = '<a class="btn-primary" href="' . esc_url($winner->getLink()) . '">Proceed with redirect</a>' .
                '<a class="btn-secondary" href="javascript:history.back()">Go back</a>';
        } else {
            $html = $this->f->str_replace('{WINNER_CLASS}', 'no-match', $html);
            $html = $this->f->str_replace('{WINNER_LABEL}', 'No match found', $html);
            $html = $this->f->str_replace('{WINNER_DETAILS}', 'No engine produced a qualifying match for this URL.', $html);

            $actionLinks = '<a class="btn-secondary" href="javascript:history.back()">Go back</a>';
        }
        $html = $this->f->str_replace('{ACTION_LINKS}', $actionLinks, $html);
