...
final class ABJ_404_Solution_DetachAbAttempt {
    ...
    /**
     * @param self::MODE_ON|self::MODE_OFF $mode Narrowed by the one parse in
     *   fromJournalRecord(); no other path constructs an attempt.
     */
    private function __construct(string $requestId, string $mode, string $part, string $payloadKey,
            int $ordinal, int $pairOrdinal, int $pairPosition, ?bool $completed) {
        $this->requestId = $requestId;
        $this->mode = $mode;
        $this->part = $part;
        $this->payloadKey = $payloadKey;
        $this->ordinal = $ordinal;
        $this->pairOrdinal = $pairOrdinal;
    ...
    public static function fromJournalRecord(array $record): ?self {
        ...
        return new self(
            $requestId,
            $mode,
            self::scalarField($record, 'part'),
            self::scalarField($record, 'payload_key'),
            self::intField($record, 'ordinal'),
            self::intField($record, 'pair_ordinal'),
            self::intField($record, 'pair_position'),
            null
        );
    }
    ...
    public function withOutcome(?bool $completed): self {
        return new self($this->requestId, $this->mode, $this->part, $this->payloadKey,
            $this->ordinal, $this->pairOrdinal, $this->pairPosition, $completed);
    }
    ...
    /**
     * One record field as a non-negative int, or -1 when it is absent or not
     * numeric. -1 is the same "no usable coordinate" sentinel the ledger
     * itself writes for an inert attempt.
     *
     * @param array<array-key, mixed> $record
     */
