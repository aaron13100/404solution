...
class ABJ_404_Solution_ViewQueriesStaged extends ABJ_404_Solution_ViewBuildCollaborator {
    ...
    // viewDoneFreshnessOptionName(), viewDoneBuiltAt(),
    // getViewDoneBuiltAtTimestamp(), viewDoneIsServeable(),
    // invalidateViewDoneServeableCache(), markViewDoneBuildCompleted(),
    // and the private $viewDoneIsServeableCache field were extracted to
    // ABJ_404_Solution_ViewDoneState (i798). Cross-collaborator callers
    // of those methods reach them through the orchestrator's __call.
    ...
    // runPageLoadFallbackAdvance() was extracted to
    // ABJ_404_Solution_ViewBuildPageLoadFallback (i798) so the admin-page-load
    // cron-recovery surface lives in its own focused class. Cross-collaborator
    // callers reach it through the orchestrator's __call.
    ...
    // sweepStaleRebuildTransients(), rebuildViewDoneInBackground(),
    // reconcileStagedTablesAtRunnerStartup(), and
    // bufferIntegrityPassesForPromote() live on the sibling
    // ABJ_404_Solution_ViewBuildRebuildReconcile collaborator. Cross-
    // collaborator calls into and out of these methods (e.g.
    // runStagedBuildOnce(), acquireViewBuildLock(),
    // markViewDoneBuildCompleted(), countViewBuildRows(),
    // countLiveRedirects()) route through the orchestrator's __call.
    ...
    //     invalidateViewDoneAndScheduleRebuild() so the next read reflects
    //     the mutation promptly, the request-lifetime serveable cache is
    //     busted, and a background rebuild is scheduled (the wall-clock gate
    //     that blocked reads was removed).
